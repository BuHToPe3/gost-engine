...
// Code inspired by OpenSSL common provider and OQS provider bio code
...
int gostprov_bio_printf(OSSL_CORE_BIO *bio, const char *format, ...)
{
    ...
    va_start(ap, format);
    ret = gostprov_bio_vprintf(bio, format, ap);
    va_end(ap);
...
static int bio_core_read_ex(BIO *bio, char *data, size_t data_len,
                            size_t *bytes_read)
{
    return gostprov_bio_read_ex(BIO_get_data(bio), data, data_len, bytes_read);
}

static int bio_core_write_ex(BIO *bio, const char *data, size_t data_len,
                             size_t *written)
{
    return gostprov_bio_write_ex(BIO_get_data(bio), data, data_len, written);
}

static long bio_core_ctrl(BIO *bio, int cmd, long num, void *ptr)
{
    return gostprov_bio_ctrl(BIO_get_data(bio), cmd, num, ptr);
}

static int bio_core_gets(BIO *bio, char *buf, int size)
{
    return gostprov_bio_gets(BIO_get_data(bio), buf, size);
}

static int bio_core_puts(BIO *bio, const char *str)
{
    return gostprov_bio_puts(BIO_get_data(bio), str);
}
...
{
    BIO_set_init(bio, 0);
    gostprov_bio_free(BIO_get_data(bio));
...
BIO *gostbio_new_from_core_bio(PROV_CTX *provctx, OSSL_CORE_BIO *corebio)
{
    ...
    if ((outbio = BIO_new(corebiometh)) == NULL)
        return NULL;
    if (!gostprov_bio_up_ref(corebio)) {
        BIO_free(outbio);
        return NULL;
    }
